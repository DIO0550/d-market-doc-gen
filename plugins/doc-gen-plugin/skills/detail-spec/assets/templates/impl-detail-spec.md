...
<!-- 実装がすべて完了したと見なすための条件チェックリスト。AIエージェントはすべての項目を満たすまで実装を終了しない -->

### 15.1 自動検証（コマンド実行で確認）

| # | 条件 | 検証コマンド | 合格基準 |
|:--|:-----|:-----------|:---------|
| D-1 | ビルドが成功する | `{ビルドコマンド}` | exit code 0・エラー出力なし |
| D-2 | 全テストがパスする | `{テストコマンド}` | 全テストケース green |
| D-3 | リント/フォーマット違反がない | `{リントコマンド}` | 警告・エラー 0件 |
| D-4 | 型チェックが通る | `{型チェックコマンド}` | 型エラー 0件 |
...
| # | 条件 | 照合先 | 確認方法 |
|:--|:-----|:-------|:---------|
| D-9 | 変更禁止事項に違反していない | セクション3 | 変更差分に禁止対象が含まれていない |
...
<!-- プロジェクト固有の完了条件があれば追記する -->

| # | 条件 | 確認方法 |
|:--|:-----|:---------|
| D-{NN} | {プロジェクト固有の条件} | {確認方法} |
