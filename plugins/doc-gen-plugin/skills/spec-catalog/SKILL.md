---
name: spec-catalog
description: 仕様書カタログスキル。19種類のテンプレートから最適な仕様書を対話的に生成する。AI実装仕様書・SRS・RFC・テスト計画・ランブック・SLO定義など幅広いドキュメント形式に対応。「仕様書を作りたい」「RFCを書きたい」「テスト計画を作りたい」「ランブックを書きたい」「SLO を定義したい」「マイグレーション計画を書きたい」「デプロイ手順を作りたい」「ポストモーテムを書きたい」「クイックスペックを書きたい」「実装計画を作りたい」「プロジェクトの CLAUDE.md を作りたい」「Arc42 で設計書を書きたい」「プロダクトブリーフを書きたい」などのリクエスト時に使用。
---

# 仕様書カタログ

19種類のテンプレートから最適な仕様書・設計書・運用ドキュメントを対話的に生成するスキル。
