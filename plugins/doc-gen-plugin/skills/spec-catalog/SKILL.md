---
name: spec-catalog
description: AIエージェント実装用の仕様書カタログスキル。22種類のテンプレートから最適な仕様書を対話的に生成する。全テンプレートはAIコーディングエージェントが読んで実装するために最適化されており、人間向けの承認フロー・ステークホルダー管理・コミュニケーション計画等は含まない。「仕様書を作りたい」「RFCを書きたい」「テスト計画を作りたい」「ランブックを書きたい」「SLO を定義したい」「マイグレーション計画を書きたい」「デプロイ手順を作りたい」「ポストモーテムを書きたい」「クイックスペックを書きたい」「実装計画を作りたい」「プロジェクトの CLAUDE.md を作りたい」「Arc42 で設計書を書きたい」「プロダクトブリーフを書きたい」「仕様を一緒に考えたい」「壁打ちしたい」「仕様を探索したい」などのリクエスト時に使用。
---

# 仕様書カタログ

22種類のテンプレートから、AIコーディングエージェントが実装するための仕様書・設計書・運用ドキュメントを対話的に生成するスキル。
...
| B6 | 設計仕様書 | `assets/templates/design/design-spec.md` | 設計判断・境界・振る舞い・エッジケースを網羅。AIが解釈の余地なく実装できる精度の設計 |
...
| C5 | 実装仕様書 | `assets/templates/implementation/impl-detail-spec.md` | 設計意図+コードレベル実装詳細。ファイル構成・関数シグネチャ・処理フロー・エラー処理まで網羅 |
| C6 | 実装前コードベース探索 | `assets/templates/implementation/impl-exploration.md` | impl-detail-spec の前段階。コードベースを探索し技術的事実を収集・記録する |
